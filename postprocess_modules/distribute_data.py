...
class DistributeData():
    ...
    def _extract_cra(self):
        ...
        self.pos_data = self.pd_data[self.pd_data['Label'].astype(str) == '1']
        self.neg_data = self.pd_data[self.pd_data['Label'].astype(str) == '-1']

        # positive data with only or without any CRA edge
        self.pos_data_cra_only = self.pos_data[self.pos_data['Predicate'].isin([self.CRTA_STR])]
        self.pos_data_except_cra = self.pos_data[~self.pos_data['Predicate'].isin([self.CRTA_STR])]
