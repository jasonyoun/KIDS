...
class DistributeData():
    ...
    def _extract_triplets(self):
        ...
        self.pos_except_target = self.pos_data[~self.pos_data[self._PRED_STR].isin([self.target_relation])]
    ...
    def _sample_negatives(self):
        ...
        for obj, count in objs_value_counts_dic.items():
            ...
            if num_known_neg_samples < num_negs_needed:
                ...
                unknown_subjs = np.array(list(set(self.all_genes.tolist()) - set(known_subjs.tolist())))
                ...
                if unknown_subjs.shape[0] < num_random_subjs:
                    ...
                    num_pos_to_remove_dic[obj] = int(np.ceil((num_random_subjs - unknown_subjs.shape[0]) / self.num_negs))
                    num_random_subjs -= (self.num_negs * num_pos_to_remove_dic[obj])

                # fill randomly samples negatives using closed world assumption
                random_generated_neg_cra = pd.DataFrame(0, index=np.arange(num_random_subjs), columns=self._SPOL_LIST)
                random_generated_neg_cra[self._SUB_STR] = np.random.choice(unknown_subjs, num_random_subjs, replace=False)
    ...
    def split_into_folds(self):
        ...
        for train_dev_index, test_index in KFold(n_splits=self.num_folds).split(self.pos_target):
            ...
            data_split_fold_dic['fold_{}_train_local_without_neg'.format(k)] = self.pos_target.iloc[train_index, :]
            data_split_fold_dic['fold_{}_dev_without_neg'.format(k)] = self.pos_target.iloc[dev_index, :]
            data_split_fold_dic['fold_{}_test_without_neg'.format(k)] = self.pos_target.iloc[test_index, :]
    ...
    def save_train_data_for_final_model(self, output_dir):
        ...
        # for each positive, randomly sample 'self.num_negs' negatives and append to 'new_with_neg_cra'
        for i in range(self.pos_target.shape[0]):
            ...
            if obj not in known_negatives_dic:
                ...
                known_negatives_dic[obj] = self.updated_neg_target[self.updated_neg_target[self._OBJ_STR].isin([obj])]

            if known_negatives_dic[obj].shape[0] < self.num_negs:
                sys.exit('We are supposed to have enough negatives now!')

            new_with_neg_cra = new_with_neg_cra.append(known_negatives_dic[obj].iloc[0:self.num_negs, :])
    ...
    def _random_sample_negs(self, cur_fold, data_split_fold_dic, dtype):
        ...
        for i in range(pos_size):
            # actual positive SPO that we're working on
            pos_triplet = data_split_fold_dic['fold_{}_{}_without_neg'.format(cur_fold, dtype)].iloc[i, :]
            obj = pos_triplet[self._OBJ_STR]
